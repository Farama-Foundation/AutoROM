...
def torrent_tar_to_buffer():
    ...
    timeit = 1
    while handle.status().state not in {4, 5}:  # finished or seeding
        # some sleep helps
        time.sleep(1)
        timeit += 1

        if timeit % 10 == 0:
            print(f"Trying to download atari roms, "
                  f"current status={status_meaning.get(handle.status().state, default='unknown')} "
                  f"({handle.status().state}), time={timeit}",
                  file=sys.stderr)
